...
// Get medication image by file ID
export const getMedicationImage = async (fileId) => {
  try {
    const response = await instance.get(`/files/${fileId}/view`, {
      responseType: "blob",
    });
    return URL.createObjectURL(response.data);
  } catch (error) {
    console.error("Failed to fetch medication image:", error);
    throw error;
  }
};
